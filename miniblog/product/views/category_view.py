...
def category_delete(request, id:int):
    categori_repository= CategoryRepository()
    categoria= categori_repository.get_by_id(id)
    categori_repository.delete(categoria)
...
def category_update(request, id:int):
    ...
    if request.method== "POST": #no hay metodo post todavia
        nombre = request.POST.get('name')
